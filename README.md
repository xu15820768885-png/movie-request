...
影巢个人 OpenAPI 试运行使用 `https://re0.me`。管理员页面恢复 OAuth 配置和固定出口测速，用于验证既有应用的 Client ID、Secret 和用户 Token；家人端资源入口与追更后台暂不启动。验证成功后再设置 `HDHIVE_BACKGROUND_ENABLED=true` 并恢复完整入口。

云服务器固定出口模板位于 `deploy/hdhive-egress`。它只代理影巢 API 的小型 HTTPS 请求，不传输影片文件；NAS 原有动态代理仍可继续用于 TMDB 等服务。

家人打开影片详情后，页面只查询癫影资源。满意的版本只有一个“转存此资源”按钮，服务端根据当前登录账号的固定配置写入115或把解锁链接发送给123。电影没有满意版本时提交求片；剧集需要缺集补齐或持续更新时，使用显眼的“提交追更/缺集需求”，管理员会在现有需求后台和通知渠道收到后续处理。
