...
未完结剧集分两步处理：先手动选择并转存初始版本，再点击“开启追更”绑定影巢原生订阅。管理员为应用追加 `subscription messages` 并重新 OAuth 后，映单低频读取本人订阅站内信；只处理115候选，使用解锁前文件列表在后台识别季集和单集大小，再把新指纹对应的文件送入 PanSave。默认单集洗版窗口为48小时、最多处理4次，窗口结束后锁定该集。权限尚未到账时继续用 `query` scope 定时查询作为兜底。
