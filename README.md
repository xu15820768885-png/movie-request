...
- 每个网站账号固定选择 115 或 123，资源页始终只有一个转存按钮；两种网盘分别使用统一转存目录
...
5. 填写 123 云盘登录手机号和密码并点击“测试登录”，然后在登录设置旁选择123统一转存文件夹。密码会加密保存在 NAS 且不会回显；升级前已经保存的 Client ID 和 Client Secret 仍可作为兼容配置继续使用。

随后在“成员账号”中为每个账号固定选择“转存115”或“转存123”。所有115账号使用115设置中的统一目录，所有123账号使用123登录旁选择的统一目录。现有账号升级后默认保持115，不会改变原来的转存目标类型；管理员账号也按同样方式配置。
